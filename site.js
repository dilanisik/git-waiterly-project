...
const server = http.createServer(async (req, res) => {
  ...
  if (publicPages.includes(req.url.split('?')[0]) || req.url.startsWith("/session.html")) {
    ...
    res.end(JSON.stringify({ success: true }));
      ...
      const hashcode = crypto.createHash('sha256').update(timeNow + Math.random().toString()).digest('hex');
  ...
  // --- BURASI ÇOK ÖNEMLİ: API ENDPOINTLERİ ---
      ...
      const idParam = parts[3]; // Keep it as a string initially

      // FIX: Construct a query that handles both numeric IDs and MongoDB ObjectIds
      let query = null;
      if (idParam) {
          // If the ID is 24 characters long, it's a MongoDB ObjectId
          if (idParam.length === 24) {
              query = { _id: new ObjectId(idParam) };
          } else {
              // Otherwise, fall back to the numeric ID
              query = { id: parseInt(idParam) };
          }
      }

      const validCollectionsAPI = ["menu", "users", "requests", "moods", "ingredients", "order_history"];
      ...
      if (validCollectionsAPI.includes(collectionName)) {
          ...
          if (req.method === "GET") {
              ...
              // FIX: Use the smart query to find the correct document
              await col.updateOne(query, { $set: updateData });
              res.writeHead(200, { "Content-Type": "application/json" });
              res.end(JSON.stringify({ success: true }));
          } 
          else if (req.method === "DELETE" && query) {
              // FIX: Use the smart query to delete the correct document
