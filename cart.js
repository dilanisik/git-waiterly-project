...
async function handlePaymentSelection(yontem) {
  ...
  // YENİ: Garsona ödeme talebini anında ilet
  const sessionHash = localStorage.getItem("sessionHash");
  try {
    await fetch("/api/session/payment-request", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ hash: sessionHash, yontem: yontem }),
    });
  } catch (e) {
    console.error("Ödeme talebi garsona iletilemedi:", e);
  }
...
async function oturumuKapat(hash, yontem) {
  // Artık oturumu kapatma isteği atmıyoruz, garsonun kendi ekranından kapatmasını bekliyoruz.
  document.getElementById("reviewStep").style.display = "none";

  // Ekrana bekleme animasyonu ve yazısı koyalım
  ...
  // Polling: Her 3 saniyede bir oturum durumunu kontrol et
  const checkInterval = setInterval(async () => {
    try {
      const res = await fetch("/api/session/current?hash=" + hash);
      // Eğer backend 404/hata döndüyse (oturum garson tarafından silindiyse), işlem tamamdır!
      if (!res.ok) {
        clearInterval(checkInterval);
        bildirimGoster(
          `Ödemeniz ${yontem} ile başarıyla alındı. Yine bekleriz! ✅`,
        );
        setTimeout(() => {
