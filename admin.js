...
function renderEmployees() {
  ...
  employeesData.forEach((emp) => {
    const statusClass =
      emp.durum === "active" ? "status-active" : "status-inactive";
...
function loadAdminIngredients() {
  ...
  Promise.all([
    ...
    .then(([ingredientsData, menuData]) => {
      ...
      if (select) {
        select.innerHTML =
          '<option value="">-- Menüden Malzeme Seçin --</option>';
        Array.from(allIngs)
          .sort((a, b) => a.localeCompare(b, "tr-TR"))
          .forEach((ing) => {
            let isAlreadyExcluded = ingredientsData.some(
              (ex) =>
                typeof ex.isim === "string" &&
                ex.isim.toLocaleLowerCase("tr-TR") ===
                  ing.toLocaleLowerCase("tr-TR"),
            );
  ...
  fetch("/api/requests")
    ...
    .then((data) => {
      ...
      data.forEach((item) => {
        ...
        const silBtn = item.istek.includes("Özel İstek")
          ? ""
          : `<button class="btn-delete" onclick="deleteRequest('${itemId}')">Sil</button>`;
...
function addNewRequest() {
  ...
  fetch("/api/requests", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ istek: text }),
  }).then(() => {
...
function deleteRequest(id) {
  openDeleteModal(id, (targetId) => {
    fetch(`/api/requests/${targetId}`, { method: "DELETE" }).then(() =>
      loadAdminRequests(),
    );
  });
}
...
function addNewMood() {
  ...
  const etiketInput = document
    .getElementById("add-mood-etiketler")
    .value.trim();
...
function editMood(id) {
  const mood = currentMoodsData.find(
  ...
  document.getElementById("add-mood-etiketler").value = Array.isArray(
    mood.etiketler,
  )
    ? mood.etiketler.join(", ")
    : "";

  document.querySelector("#mood-add-form h3").innerText =
    "✏️ Ruh Halini Düzenle";
  document
    .querySelector("#mood-add-form .btn-submit")
    .setAttribute("onclick", "saveMood()");
}

function saveMood() {
  const isim = document.getElementById("add-mood-isim").value.trim();
  const etiketler = document
    .getElementById("add-mood-etiketler")
    .value.split(",")
    .map((e) => e.trim().toLowerCase());
  fetch(`/api/moods/${editingMoodId}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      isim,
      emoji: document.getElementById("add-mood-emoji").value,
      etiketler,
    }),
  }).then(() => {
    resetMoodForm();
...
function deleteMood(id) {
  openDeleteModal(id, (targetId) => {
    fetch(`/api/moods/${targetId}`, { method: "DELETE" }).then(() =>
      loadAdminMoods(),
    );
  });
}
...
function resetMoodForm() {
  ...
  document
    .querySelector("#mood-add-form .btn-submit")
    .setAttribute("onclick", "addNewMood()");
}
...
function loadAdminMenu() {
  fetch("/api/menu")
    ...
    .then((data) => {
      ...
      data.forEach((item) => {
        const icerikMetni =
          item.icerik && Array.isArray(item.icerik)
            ? item.icerik.join(", ")
            : "-";
        const itemId = item._id || item.id;
...
function editMenu(id) {
  const item = currentMenuData.find(
    (i) => String(i.id) === String(id) || String(i._id) === String(id),
  );
  if (!item) return;
  editingMenuId = id;
  ...
  document.getElementById("add-alerjenler").value =
    item.alerjenler && Array.isArray(item.alerjenler)
  ...
  document.querySelector("#panel-menu .add-form h3").innerText =
    "✏️ Ürün Düzenle";
}
...
function saveMenuItem() {
  ...
  const icerik = document
    .getElementById("add-icerik")
    .value.split(",")
    .map((i) => i.trim())
    .filter((i) => i);
  const aciklama = document.getElementById("add-aciklama").value.trim();
  const alerjenler = document
    .getElementById("add-alerjenler")
    .value.split(",")
    .map((a) => a.trim())
    .filter((a) => a);
  const tags = document
    .getElementById("add-etiketler")
    .value.split(",")
    .map((e) => e.trim())
    .filter((e) => e);
  const vegan = document.getElementById("add-vegan")
    ? document.getElementById("add-vegan").value === "true"
    : false;

  let resim = document.getElementById("add-resim")
    ? document.getElementById("add-resim").value.trim()
    : "/images/americano.jpg";
  if (typeof menuImageBase64 !== "undefined" && menuImageBase64) {
    resim = menuImageBase64;
  }

  if (!isim || icerik.length === 0 || !kategori)
    return alert("İsim, Kategori ve İçerik zorunludur!");
...
function resetMenuForm() {
  ...
  document.getElementById("add-etiketler").value = "";
  if (document.getElementById("add-vegan"))
    document.getElementById("add-vegan").value = "false";
  if (document.getElementById("add-resim"))
...
function deleteItem(id) {
  openDeleteModal(id, (targetId) => {
    fetch(`/api/menu/${targetId}`, { method: "DELETE" }).then(() =>
      loadAdminMenu(),
    );
  });
}
...
async function loadOrders() {
  ...
  [...todaysOrders].reverse().forEach((order) => {
    const urunDetay =
      order.urunler && Array.isArray(order.urunler)
        ? order.urunler
            .map((u) => {
              let urunIsmi = u.isim;

              if (!urunIsmi || String(urunIsmi).length >= 20 || u.id) {
                const menuUrun = currentMenuData.find(
                  (m) =>
    ...
    if (order.tarih) {
      ...
      if (!isNaN(d.getTime())) {
        ...
        formattedTime = d.toLocaleTimeString("tr-TR", {
          hour: "2-digit",
          minute: "2-digit",
        });
      }
...
function addManualOrder() {
  ...
  if (!urunlerStr || !tutar)
    return alert("Ürünler ve Tutar alanları zorunludur!");
