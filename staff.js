...
function renderTables(tables) {
  ...
  tables.forEach((table) => {
    ...
    const isPaymentPending = session && session.odemeTalebi; // Backend'den gelecek yeni alan

    let requestsHTML = "";
    if (hasRequest || isPaymentPending) {
      let badgesHTML = "";

      // Normal garson çağırma talepleri
      if (hasRequest) {
        badgesHTML += session.talepler
          .map((t) => `<span class="req-badge">${t}</span>`)
          .join("");
      }
      // Ödeme talebi rozeti
      if (isPaymentPending) {
        badgesHTML += `<span class="req-badge" style="background:#ff9800; border: 1px solid #e65100;">💸 ${session.odemeTalebi} Bekliyor</span>`;
      }
