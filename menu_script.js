...
document.addEventListener('DOMContentLoaded', () => {
    fetch('/api/menu')
        ...
        .catch(err => {
            ...
            document.getElementById("menu-list").innerHTML = "<p style='color:red;'>Menü yüklenirken bir hata oluştu.</p>";
        });
});
...
function renderCategories() {
    ...
    const uniqueCategories = ['Hepsi', ...new Set(menuData.map(item => item.kategori).filter(k => k && k.trim() !== ''))];
...
function renderMenu() {
    ...
    const filteredData = menuData.filter(item => {
        // Kategori Filtresi
        if (activeCategory !== 'Hepsi' && item.kategori !== activeCategory) return false;
        ...
        if (unwantedAllergens.size > 0 && item.alerjenler && Array.isArray(item.alerjenler)) {
    ...
    if (filteredData.length === 0) {
        list.innerHTML = "<p style='color:gray; padding: 20px; grid-column: 1/-1;'>Seçtiğiniz filtrelere uygun ürün bulunamadı. 😔</p>";
        return;
    }
    ...
    filteredData.forEach(item => {
        ...
        let veganBadge = item.vegan ? `<span style="font-size: 10px; background: #e8f5e9; color: #2e7d32; padding: 3px 6px; border-radius: 8px; border: 1px solid #a5d6a7; margin-left:8px; vertical-align: middle;">🌱 Vegan</span>` : "";
...
function openItemModal(id) {
    ...
    document.getElementById("modal-img").src = item.resim || '/images/americano.jpg';
...
function clearFilters() {
    ...
    document.querySelectorAll(".allergen-selected").forEach(el => el.classList.remove("allergen-selected"));
